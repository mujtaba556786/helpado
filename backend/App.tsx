...
import { User, Booking, Review, DashboardStats } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [u, b, st, rv] = await Promise.all([
        apiService.getUsers(),
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-20 shadow-sm">
          <div className="flex items-center space-x-4">
            ...
            <div className="px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest bg-[#4FB584]/10 text-[#2F855A]">
              Admin Console
            </div>
          </div>
          <div className="flex items-center space-x-4">
             <div className="flex flex-col items-end">
                ...
                <span className="text-[10px] font-black text-[#2F855A] uppercase tracking-widest">{currentUser.email}</span>
             </div>
