...
const UserManagementView: React.FC<UserManagementProps> = ({ users, setUsers }) => {
  ...
  // GDPR erasure: the row stays (bookings/ratings reference it) but every
  // personal field is blanked and the account can no longer log in.
  const handleErase = async (user: User) => {
    const typed = window.prompt(`Delete "${user.name}" permanently?\nProfile, messages and reviews are removed and the account cannot log in again. This cannot be undone.\n\nType DELETE to confirm.`);
    if (typed !== 'DELETE') return;
    const ok = await apiService.eraseUser(user.id);
    if (ok) {
      setUsers(prev => prev.map(u => u.id === user.id ? { ...u, name: 'Gelöschter Nutzer', email: '', status: UserStatus.DELETED } : u));
    } else {
      window.alert('Could not delete this user (admin accounts cannot be erased here).');
    }
  };
                ...
                <td className="px-8 py-5">
                   <div className="flex items-center justify-end space-x-2">
                       ...
                       {user.status !== UserStatus.DELETED && (
                         <button
                          onClick={() => handleErase(user)}
                          className="px-4 py-2 bg-slate-100 text-rose-600 rounded-xl text-[10px] font-black uppercase hover:bg-rose-600 hover:text-white transition-colors"
